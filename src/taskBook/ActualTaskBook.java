...
public class ActualTaskBook {
    ...
    private Task newTask() {
        try {
            try {
                ...
                TypesTask typesTask;
                {
                    if (type != 1) typesTask = TypesTask.WORKING;
                    else typesTask = TypesTask.PERSONAL;
                }
                System.out.println("Выберите сроки выполнения задачи: " + "1. разовая" +
                        "2. ежедневно; " +
                        "3. еженедельно; " +
                        "4. ежемесячно; " +
                        "5. ежегодично. ");
                ...
                System.out.println("Введите дату и время задания в виде <год-месяц-день-часы-минуты>");
                LocalDateTime time = LocalDateTime.of(scanner.nextInt(), scanner.nextInt(),
                ...
                {
                    if (frequency == 1)
                        return new OneTime(title, description, typesTask, time);
                    else if (frequency == 2)
                        return new Daily(title, description, typesTask, time);
                    else if (frequency == 3)
                        return new Weekly(title, description, typesTask, time);
                    else if (frequency == 4)
                        return new Monthly(title, description, typesTask, time);
                    else if (frequency == 5)
                        return new Yearly(title, description, typesTask, time);
                }
    ...
    public void createNewTask() {
        Task task = newTask();
        {
            if (task != null) actualTaskBook.put(task.getId(), task);
            else System.out.println("Задача не была добавлена. Данные были введены некорректно");
        }
    }
        ...
        try {
            try {
                ...
                for (Map.Entry<Long, Task> pair : actualTaskBook.entrySet()) {
                    ...
                    } else if (pair.getValue().getClass().equals(OneTime.class)) {
    ...
    public void sortActualTaskBookByTime() {
        ...
        for (Map.Entry<Long, Task> pair : actualTaskBook.entrySet()) {
            if(!mapForSortedTasksByTime.containsKey(pair.getValue().getTime().toLocalDate()))
                mapForSortedTasksByTime.put(pair.getValue().getTime().toLocalDate(), pair.getValue().getInfo());
            else {
        ...
        for (Map.Entry<LocalDate, String> pair: mapForSortedTasksByTime.entrySet())
            System.out.println(pair);
    }
    @Override
